...
from .functions import main, total, average_score, sec_average_score, sec_total, nur_average_score, nursery_total
from . import db
from .models import Student
import json

views = Blueprint('views', __name__)   
@views.route('/nursery', methods=['GET', 'POST'])
def nursery_grade():
    name_of_student = ''
    average = ''
    total_score = '' 
    ...
    if request.method == 'POST':
        ...
        nur_sci_test = request.form.get('nur-sci-test')
        nur_sci_exam = request.form.get('nur-sci-exam')
        nur_sci_test, nur_sci_exam, nur_sci_total = main(nur_sci_test,nur_sci_exam)
        ...
        soc_hab_test = request.form.get('soc-hab-test')
        soc_hab_exam = request.form.get('soc-hab-exam')
        soc_hab_test, soc_hab_exam, soc_hab_total = main(soc_hab_test,soc_hab_exam)

        health_hab_test = request.form.get('health-hab-test')
        health_hab_exam = request.form.get('health-hab-exam')
        health_hab_test, health_hab_exam, health_hab_total = main(health_hab_test,health_hab_exam)
        ...
        writing_test = request.form.get('writing-test')
        writing_exam = request.form.get('writing-exam')
        writing_test, writing_exam, writing_total = main(writing_test,writing_exam)

        total_score = nursery_total(math_total, eng_total, nur_sci_total, crs_total, computer_total, igbo_total, cca_total, verbal_total, quant_total, writing_total, health_hab_total, soc_hab_total)
        average = nur_average_score(total_score)


    return render_template('nursery-grade.html',name_of_student=name_of_student, average=average, total_score=total_score, math_test=math_test, math_exam=math_exam, math_total=math_total,
                            eng_test=eng_test, eng_exam=eng_exam, eng_total=eng_total, nur_sci_test=nur_sci_test, nur_sci_exam=nur_sci_exam, nur_sci_total=nur_sci_total, cca_exam=cca_exam, 
                            cca_test=cca_test, cca_total=cca_total,igbo_test=igbo_test, igbo_exam=igbo_exam, igbo_total=igbo_total, quant_test=quant_test, quant_exam=quant_exam, 
                            quant_total=quant_total, verbal_test=verbal_test, verbal_exam=verbal_exam, verbal_total=verbal_total,computer_exam=computer_exam, computer_test=computer_test, computer_total=computer_total,
                            writing_test=writing_test, writing_exam=writing_exam, writing_total=writing_total, crs_test=crs_test, crs_exam=crs_exam, crs_total=crs_total, soc_hab_test=soc_hab_test, soc_hab_exam=soc_hab_exam, soc_hab_total=soc_hab_total,
                            health_hab_test=health_hab_test, health_hab_exam=health_hab_exam, health_hab_total=health_hab_total)
...
def seccondary_grade():
    ...
    if request.method == 'POST':
        ...
        history_test = request.form.get('history-test')
        history_exam = request.form.get('history-exam')
        history_test, history_exam, history_total = main(history_test,history_exam)


        sec_total_score = sec_total(math_total, eng_total, basic_tech_total, Basic_sci_total, Agric_total, igbo_total, cca_total, 
                                    Homec_total, business_studies_total, lit_total, computer_total, phe_total, crs_total, civic_total, 
                                    social_studies_total, history_total)
    ...
    return render_template('sec-grade.html', students = Student.query.all(), name_of_student=name_of_student, sec_average=sec_average, sec_total_score=sec_total_score, math_test=math_test, math_exam=math_exam, math_total=math_total, eng_test=eng_test, eng_exam=eng_exam, social_studies_test=social_studies_test, crs_test=crs_test,
                           ...
                           lit_test=lit_test, lit_exam=lit_exam, lit_total=lit_total, history_test=history_test, history_exam=history_exam, history_total=history_total, Civic_test=civic_test, Civic_exam=civic_exam, Civic_total=civic_total, computer_test=computer_test, computer_exam=computer_exam, computer_total=computer_total)
